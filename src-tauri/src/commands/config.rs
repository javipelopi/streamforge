...
// ============================================================================
// Standalone CLI helpers (no Tauri State required)
// ============================================================================

/// Export configuration to JSON string using a DbConnection directly.
///
/// This is the CLI-friendly version of `export_configuration` — it takes a
/// `&DbConnection` instead of `tauri::State<DbConnection>`.
...
///
/// This is the CLI-friendly version of `import_configuration`.
pub fn import_configuration_standalone(
    ...
    .map_err(|e| {
        ...
        if matches!(e, diesel::result::Error::RollbackTransaction) {
            ConfigError::ImportFailed(
                "Import validation failed: One or more records have empty required fields".to_string(),
            )
        } else {
            ConfigError::DatabaseError(error_str)
        }
