...
interface DraggableChannelRowProps {
  ...
  onMouseDragStart?: (channelId: number) => void;
  onMouseDragOver?: (channelId: number) => void;
  onKeyboardPick?: (channelId: number) => void;
  onKeyboardDrop?: () => void;
...
/**
 * DraggableChannelRow component using native HTML5 drag-and-drop
 * This implementation works with Playwright's dragTo() method
 *
...
export const DraggableChannelRow = memo(function DraggableChannelRow({
  ...
  // Mouse event handlers for Playwright tests (using page.mouse.down/hover)
  const handleMouseDown = useCallback((e: MouseEvent<HTMLDivElement>) => {
    // Only handle left mouse button
    if (e.button === 0 && onMouseDragStart) {
      onMouseDragStart(channel.id);
    }
  }, [channel.id, onMouseDragStart]);

  // Handle mouse enter on the row for drop target highlighting
  const handleMouseEnter = useCallback(() => {
    // Only highlight if another channel is being mouse-dragged
    if (isMouseDragging && onMouseDragOver) {
      onMouseDragOver(channel.id);
    }
  }, [channel.id, isMouseDragging, onMouseDragOver]);

  // Keyboard handler for accessibility (Space to pick/drop, Arrow keys to move)
  const handleKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>) => {
    if (e.key === ' ' || e.key === 'Space') {
      ...
      if (isKeyboardPicked) {
        ...
        // Pick up the item
        if (onKeyboardPick) {
          onKeyboardPick(channel.id);
        }
      }
    } else if (isKeyboardPicked && (e.key === 'ArrowUp' || e.key === 'ArrowDown')) {
      e.preventDefault();
      if (onKeyboardMove) {
        onKeyboardMove(channel.id, e.key === 'ArrowUp' ? 'up' : 'down');
      }
    }
  }, [channel.id, isKeyboardPicked, onKeyboardPick, onKeyboardDrop, onKeyboardMove]);
